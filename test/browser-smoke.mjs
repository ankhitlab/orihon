...
async function readInitial(entry) {
  return (await Promise.all(manifest.initialLoads[entry].files.map(file => readFile(new URL(`../dist/${file}`, import.meta.url), 'utf8')))).join('\n');
}
const coreLoader = await readInitial('orihon.core.esm.js');
const standardLoader = await readInitial('orihon.standard.esm.js');
...
const advancedLoader = await readInitial('orihon.esm.js');
